...
TEST(argparse, get_option_index) {
    ...
    // NULL INDEX TEST
    EXPECT_EQ(get_option_index(NULL, (const Option**)define_opt, define_opt_len, (const char*)"a"), _FAILED);
    ...
    // TEST 1-1
    EXPECT_EQ(get_option_index(&index, (const Option**)define_opt, define_opt_len, (const char*)"a"), _SUCCESS);
    EXPECT_EQ(index, 0);
    // TEST 1-2
    EXPECT_EQ(get_option_index(&index, (const Option**)define_opt, define_opt_len, (const char*)"b"), _SUCCESS);
    EXPECT_EQ(index, 1);
    // TEST 1-3
    EXPECT_EQ(get_option_index(&index, (const Option**)define_opt, define_opt_len, (const char*)"c"), _SUCCESS);
    EXPECT_EQ(index, 2);
    // TEST 1-4
    EXPECT_EQ(get_option_index(&index, (const Option**)define_opt, define_opt_len, (const char*)"d"), _SUCCESS);
    EXPECT_EQ(index, _NOT_FOUND);
}
